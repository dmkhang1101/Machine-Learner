...
with open('./cancer.csv') as csvfile:
    for row in csvfile:
        ...
        label = new_row[-1]
        data = list(map(float,row.strip().split('\t')[:-1]))
        data.insert(0,label) #type: ignore
        cancer.append(data)

normalized_cancer = [normalize_features(data,cancer) for data in cancer]
...
df.to_csv('./house_votes_84_encoded.csv')

house_votes_encoded = []

with open('./house_votes_84_encoded.csv') as csvfile:
    not_data = True
    for row in csvfile:
        if not_data:
            not_data = False
...
ten_folds_cancer = construct_10_folds(normalized_cancer,['0','1'],-1)
...
# Stopping Criteria : Stop after 10 iterations.
# Step cost : 0.1
# compute_performance(ten_folds_cancer,0.0001,[9,4,2],['0','1']) #Accuracy: 0.9656, F1: 0.9642
# compute_performance(ten_folds_cancer,0,[9,5,2],['0','1']) #Accuracy: 0.9643, F1: 0.9616
# compute_performance(ten_folds_cancer,0.0001,[9,8,2],['0','1']) #Accuracy: 0.9650, F1: 0.9635
# compute_performance(ten_folds_cancer,0.0001,[9,4,3,2],['0','1']) #Accuracy: 0.9652, F1: 0.9639
# compute_performance(ten_folds_cancer,0.0001,[9,8,6,2],['0','1']) #Accuracy: 0.9644, F1: 0.9625
# compute_performance(ten_folds_cancer,0.001,[9,8,4,2],['0','1']) #Accuracy: 0.9470, F1: 0.9471
...
training_cancer = []
for fold in ten_folds_cancer[1:]:
    training_cancer.extend(fold)
test_cancer = prepare_training(ten_folds_cancer[0],['0','1'])
training_cancer = prepare_training(training_cancer,['0','1'])
weights_cancer = initialize_weights([9,4,2])
lamb_cancer = 0.0001
...
# result_cancer = plot(test_cancer,training_cancer,weights_cancer,lamb_cancer,['0','1'])
# x_cancer = result_cancer[0]
# J_cancer = result_cancer[1]
# plt.plot(x_cancer,J_cancer)
# plt.xlabel("Number of instances trained")
# plt.ylabel("Cost J")
# plt.title("Cost J against number of training instance for cancer dataset")
# plt.show()